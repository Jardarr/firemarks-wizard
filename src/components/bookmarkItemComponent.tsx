...
const BookmarkItemComponent: React.FC<BookmarkItemComponentProps> = ({ item, expandedFolders, toggleFolder, onUpdate, darkMode }) => {
	...
	return (
		<div className={`border rounded-sm min-w-0 ${darkMode.value ? "border-indigo-500 bg-indigo-950/50" : "border-amber-500 bg-amber-950/50"}`}>
			<strong className="m-2 flex cursor-pointer items-center justify-between" onClick={() => toggleFolder(item.guid)}>
			...
			<AnimatePresence>
				{item.children && item.children.length > 0 && expandedFolders[item.guid] && (
					<motion.div
						...
						<BookmarkTree items={item.children} isRoot={false} parentId={item.guid} onUpdate={onUpdate} darkMode={darkMode} expandedFolders={expandedFolders} toggleFolder={toggleFolder} />
					</motion.div>
				)}
			</AnimatePresence>
		</div>
